"""The declared version, held against the artifact that would make it true.

``pyproject.toml`` declares ``version = "0.1.0"`` and ``git tag --list`` is
empty, here and on ``origin``. Nothing was ever tagged, so the number names no
artifact: a reader who takes ``0.1.0`` at face value can install nothing with
it. The Release & Versioning row already records that ("no tag cut, no signed
tag, no release workflow, no published artifact") and nothing enforced the
sentence, which is the shape every other stale claim in this repository had
before a test was pointed at it.

The wider measurement, 2026-09-06: twenty public repositories in this portfolio
declare a version in ``pyproject.toml`` that nothing was ever tagged for. It is
the portfolio's dominant defect class, a value published where there is nothing
behind it, applied to releases rather than to a count.

Being unreleased is not the defect. Publishing the number in silence is. So the
rule sorts the declared version into one of two states.

*No tag exists.* Legitimate, and it passes, but only while the repository says
so where a reader arrives: the ``**Status:**`` line, or the
``Release & Versioning`` row of the conformance table. The sentence has to name
the version ``pyproject.toml`` declares. Naming it is what stops the disclosure
going stale, because a bump to ``0.2.0`` under a README still explaining
``0.1.0`` reads exactly like a current disclosure to anyone who does not diff
the two.
...
This repository is in the first state, so the second is exercised from
synthetic input on every run rather than waiting for a bad day. A gate whose
failing branch never executes is a gate nobody has evidence about, which is the
category ``tests/test_a11y_gate.py`` already guards against by running the
accessibility tooling over pages that must fail it. ``release_claim_failure``
is a pure function of the three things it reads, and
``test_the_rule_reads_this_readme_rather_than_passing_regardless`` closes the
remaining gap by sabotaging the real README and requiring the real check to
notice, asserting the sabotage landed before reading the result.
...
file exists to prevent. ``actions/checkout`` fetches no tags at its default
depth, so the job running this suite checks out with ``fetch-depth: 0``.

``package.json`` is not bound to the declared version and should not be. It is
``perimeter-page-checks``, ``"private": true`` at ``0.0.0``, the npm
convention for a package that is not published; nothing in it ships in the
site. ``test_the_page_check_package_stays_unpublishable`` holds it there, so it
cannot quietly acquire a version number of its own and become a second release
claim nobody is watching.

One consequence worth knowing before the first release: cut the signed tag on
the same commit that declares the version. Merging a version bump ahead of its
tag puts ``main`` into the failing state above, accurately, because for that
window the repository really does declare a version nothing was tagged for.
"""
...
_VERSION_TAG = re.compile(
    r"^v?(?P<version>[0-9]+(?:\.[0-9]+)*(?:[.\-+][0-9A-Za-z.\-+]+)?)$"
)
...
def tag_authority_failure(
    *, inside_work_tree: bool, shallow: bool, tag_option: str
) -> str | None:
...
def release_claim_failure(
    declared: str, tags: Sequence[str], readme: str
) -> str | None:
    ...
    for _, statement in places:
        ...
        if declared in statement and any(
            marker in lowered for marker in UNRELEASED_MARKERS
        ):
            return None
    return (
...
def test_the_authority_check_refuses_every_checkout_that_cannot_see_tags() -> None:
    ...
    """
    assert (
        tag_authority_failure(inside_work_tree=True, shallow=False, tag_option="")
        is None
    )
    assert (
        tag_authority_failure(inside_work_tree=False, shallow=False, tag_option="")
        is not None
    )
    assert (
        tag_authority_failure(inside_work_tree=True, shallow=True, tag_option="")
        is not None
    )
    assert (
        tag_authority_failure(
            inside_work_tree=True, shallow=False, tag_option="--no-tags"
        )
        is not None
    )
...
def test_every_restatement_of_the_version_agrees_with_pyproject() -> None:
    ...
    assert not disagreeing, f"pyproject.toml declares {declared}; " + "; ".join(
        disagreeing
    )
...
def test_citation_declares_a_release_date_only_for_a_version_that_was_tagged() -> None:
    ...
    dated = [
        line
        for line in _read(CITATION).splitlines()
...
def test_the_changelog_patterns_tell_a_dated_section_from_an_unreleased_one() -> None:
    ...
    dated = "# c\n\n## [1.2.3] - 2026-01-02\n\n- a change\n"
    unreleased = "# c\n\n## [Unreleased]\n\n- a change\n"
    assert [found.group("version") for found in _DATED_SECTION.finditer(dated)] == [
        "1.2.3"
    ]
...
def test_the_page_check_package_stays_unpublishable() -> None:
    """``package.json`` is dev tooling, not a second thing with a version.

    It is ``"private": true`` at ``0.0.0``, npm's convention for a package that
    is never published, and nothing in it ships in the built site. That is why
    it is absent from ``RESTATEMENTS``. Held here so the exemption stays true:
    a real version number appearing in it would be a release claim with nothing
    behind it, in the one file this suite had decided not to look at.
    """
    package = json.loads(_read(REPO_ROOT / "package.json"))
    assert package.get("private") is True, "package.json is no longer private"
    assert package["version"] == "0.0.0", (
        f"package.json now declares version {package['version']!r}. If it is meant "
        "to be published, add it to RESTATEMENTS; if not, put it back to 0.0.0"
    )
